        ...
        public Status deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
            ...
            if (tree instanceof TextNode) {
                ...
                if (VALIDATING_VARIANT_CLASS_NAME.equals(variantName)) {
                    ...
                    final var errorName = String.format("Expected variant name one of %s or %s,but got %s", VALIDATING_VARIANT_CLASS_NAME, COMMITTED_VARIANT_CLASS_NAME, variantName);
                ...
                throw new JsonParseException(p, "Expected status is represented as text or Json object, but node was " + tree.getClass());
            }
        }
    }
}